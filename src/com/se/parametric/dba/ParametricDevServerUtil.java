...
//		partsParametric.setComponent(component);
	 ...
	 * @throws Exception
	 */
	// public static void savePkgData(List<Component> componentList,
	// Map<Pl, Map<String, Map<String, List<String>>>> plDataMap,
	// Supplier supplier, Document document) throws Exception {
	...
	// //
	// try {
	// Component component = getComponentByPartInfo(partInfo,
	// componentList);
	// // no change
	// Map<String, List<String>> featureMap = dataMap
	// .get(partInfo);
	// //
	// Set<String> featureNames = featureMap.keySet();
	// Map<Long, PkgFeature> featureGroupMap = new HashMap<Long, PkgFeature>();
	//
	// Document pkgUrlDocument = null;
	// PkgJedec pkgJedec = null;
	// Date processingDate = null;
	//
	// for (String pkgFeatureName : featureNames) {
	// List<String> valuesList = featureMap
	// .get(pkgFeatureName);
	// if (valuesList == null || valuesList.size() == 0)
	// continue;
	// PkgFeature pkgFeature = ParaQueryUtil
	// .getPkgFeatureByExactName(pkgFeatureName, 2,
	// session);
	//
	// int num = pkgFeature.getId().intValue();
	//
	// String value = valuesList.get(0);
	// if (num == 39) {
	// // document
	// pkgUrlDocument = ParaQueryUtil
	// .getDocumentBySeUrl(value, session);
	// } else if (num == 36) {
	// // jedec
	// String[] valueOfString = value.split("\\/");
	// if (valueOfString.length == 2) {
	// String standard = valueOfString[0];
	// String variation = valueOfString[1];
	// pkgJedec = ParaQueryUtil
	// .getPkgJedecByStandardAndVariation(
	// standard, variation, session);
	// }
	//
	// } else if (num == 40) {
	// // processingDate
	// processingDate = getDate(value);
	// } else {
	// long[] ret = savePkgFeatureValues(
	// /* component, */valuesList, pkgFeature, document,
	// session);
	// long groupId = ret[0];
	// if (ret[1] == 1)
	// isNeedValueApproval = true;
	// featureGroupMap.put(groupId, pkgFeature);
	// }
	// }
	// // save parts pkg object
	// savePkgPartObject(component, featureGroupMap,
	// pkgUrlDocument, pkgJedec, processingDate, session);
	// savedParts++;
	// } catch (Throwable e) {
	// e.printStackTrace();
	...
	//
	// exceptionString = exceptionString.length() == 0 ? exceptionString
	// : exceptionString + ",";
	// exceptionString += "SAVED_PARTS:" + savedParts + ":"
	// + pl.getName();
	//
	// }
	// } // end of pl loop
	...
	// changePkgTaskStatus(document, "Pending TL Review", null);
	// }
	...
	// throws Exception {
	// long[] ret = new long[2];
	// // values do not have a group, so create group for them
	// //
	// ret[1] = 0;
	...
	//
	// valuesList.remove(0);
	// // get approved values list
	// List<PkgApprovedValue> pkgApprovedValuesList = new ArrayList<PkgApprovedValue>();
	// for (String valueStr : valuesList) {
	// if (valueStr == null || valueStr.equals(""))
	// continue;
	// PkgApprovedValue pkgApprovedVal = ParaQueryUtil
	// .getApprovedPkgFeatureValue(valueStr, pkgFeature.getName(),
	// pkgFeature.getQualityApproved(), session);
	// // check if the value found in the approved value or not
	// // if not found and the quality approved flag not true
	// // then add it to value and approved value
	// if (pkgApprovedVal == null) {
	// // add pkg approved value
	// PkgValue pkgValue = ParaQueryUtil.getPkgValue(
	// valueStr, session);
	//
	// pkgApprovedVal = addApprovedPkgFeatureValue(pkgValue,
	// pkgFeature, pkgFeature.getQualityApproved(), session);
	// // save in tracking need approval
	// if (pkgFeature.getQualityApproved()) {
	// // check pkgfeature is equal true change status need value
	// // approval
	// changePkgTaskStatus(document, "Need Value Approval", null);
	// // insert new tracking pkg approved value status pending
	// ParaQueryUtil.insertNewTrackingPkgApprovedValue(
	// pkgApprovedVal, session);
	// // insert new tracking pkg document approved value status
	// // pending
	// ParaQueryUtil
	// .insertNewTrackingPkgDocumentApprovedValue(
	// pkgApprovedVal, document,/* component, */
	// session);
	// ret[1] = 1;
	// }
	//
	// }
	// pkgApprovedValuesList.add(pkgApprovedVal);
	// }
	// // check if these values have a group
	// long groupId = getPkgGroupByPkgValues(pkgApprovedValuesList, session);
	// if (groupId > 0) {
	// ret[0] = groupId;
	// return ret;
	// }
	//
	// //
	// groupId = QueryUtil.getRandomID();
	// for (PkgApprovedValue pkgApprovedValue : pkgApprovedValuesList) {
	// // save pkg values group in pkg value group
	// savePkgValuesGroup(session, groupId, pkgApprovedValue);
	// }
	//
	// ret[0] = groupId;
	// return ret;
	 ...
	 * 
	 * 
	 * @param approvedValuesList
	 * @param session
	 * @return
	 */

	public static void updateTrackingPKGDocApprovale(long trackingPkgDocApprovid, String approvalStatus, String comentTextBox) throws Exception
	{
		Session session = SessionUtil.getSession();
		try
		{
			// TrackingPkgDocApprov trackingPkgDocApprov =
			// ParaQueryUtil.getTrackingPkgDocApproval(session,
			// docuomentId, pkgApprovedValue);
			TrackingPkgDocApprov trackingPkgDocApprov = (TrackingPkgDocApprov) session.get(TrackingPkgDocApprov.class, trackingPkgDocApprovid);
			TrackingTaskStatus trackingTaskStatus = ParaQueryUtil.getTrackingTaskStatus(session, approvalStatus);
			if(approvalStatus.equals("Pending"))
			{
				trackingPkgDocApprov.setTrackingTaskStatus(trackingTaskStatus);
				trackingPkgDocApprov.setTrackingTaskQaStatus(null);
				ParaQueryUtil.updateTrackingPkgDocApprov(session, trackingPkgDocApprov);

			}
			else if(approvalStatus.equals("Send Back To Developer"))
			{

				TrackingPkg trackingPkg = ParaQueryUtil.getTrackingPkg(session, trackingPkgDocApprov.getDocument().getId());
				TrackingFeedback trackingFeedback = ParaQueryUtil.getTrackingFeedback(trackingPkg.getPkgUserId(), "PKG", trackingPkg.getDocument(), session);
				if(trackingFeedback != null)
				{
					if(trackingFeedback.getComments() != null && !trackingFeedback.getComments().trim().equals(""))
						trackingFeedback.setComments(trackingFeedback.getComments() + " | " + comentTextBox);
					else
						trackingFeedback.setComments(comentTextBox);

					trackingFeedback.setFeedbackDate(new Date());
					ParaQueryUtil.updateTrackingFeedback(session, trackingFeedback);
				}
				else
				{
					trackingFeedback = new TrackingFeedback();
					trackingFeedback.setId(System.nanoTime());
					Date date = trackingPkg.getAssignedDate();
					if(date != null)
					{

						System.out.println("DeliveryDate = " + date.getTime());
						...
						trackingFeedback.setAssignedDate(addDay(date));
					}
					trackingFeedback.setUserId(trackingPkg.getPkgUserId());
					trackingFeedback.setComments(comentTextBox);
					trackingFeedback.setDocument(trackingPkg.getDocument());
					trackingFeedback.setFeedbackDate(new Date());
					trackingFeedback.setFinishedDate(trackingPkg.getFinishedDate());
					trackingFeedback.setPl(trackingPkg.getPl());
					trackingFeedback.setPrioriy(trackingPkg.getPrioriy());
					trackingFeedback.setQaReviewDate(trackingPkg.getQaReviewDate());
					trackingFeedback.setQaUserId(trackingPkg.getQaUserId());
					trackingFeedback.setQaUserName(trackingPkg.getQaUserName());
					trackingFeedback.setRecieveDate(trackingPkg.getRecieveDate());
					trackingFeedback.setRequestId(trackingPkg.getRequestId());
					trackingFeedback.setTrackingFeedbackType(ParaQueryUtil.getTrackingFeedbackTypeByexactName("Internal", session));
					trackingFeedback.setTrackingTaskStatus(trackingTaskStatus);
					trackingFeedback.setTrackingTaskType(trackingPkg.getTrackingTaskType());
					trackingFeedback.setTrackingTeam(ParaQueryUtil.getTrackingTeamByExactName("PKG", session));
					ParaQueryUtil.insertNewFeedBack(session, trackingFeedback);
				}
				trackingPkgDocApprov.setTrackingTaskStatus(trackingTaskStatus);
				trackingPkgDocApprov.setTrackingTaskQaStatus(null);
				ParaQueryUtil.updateTrackingPkgDocApprov(session, trackingPkgDocApprov);
			...
			else if(approvalStatus.equals("Send To QA"))
			{

				trackingPkgDocApprov.setTrackingTaskStatus(trackingTaskStatus);
				TrackingTaskQaStatus trackingTaskQaStatus = ParaQueryUtil.getTrackingTaskQaStatus(session, "Pending");
				trackingPkgDocApprov.setTrackingTaskQaStatus(trackingTaskQaStatus);
				ParaQueryUtil.updateTrackingPkgDocApprov(session, trackingPkgDocApprov);
	...
	@SuppressWarnings("unchecked")
	private static List<TrackingParamDocApprov> getTrackingParamDocApprovByGroupId(Long approvedGroupId, Session session)
	{
		final Criteria crit = session.createCriteria(TrackingParamDocApprov.class);
		crit.add(Restrictions.eq("approvedGroupId", approvedGroupId));
		List<TrackingParamDocApprov> trackingParamDocApprovs = crit.list();
		return trackingParamDocApprovs;
	}

	public static void updateTrackingParamDocApprovByTrackingTaskQaStatus(String approvalStatus, String comments, List<TrackingParametricDocApprovaleDTO> listTrackingParametricDocApprovaleDTOs) throws Exception
	{
		Session session = SessionUtil.getSession();
		try
		{

			for(TrackingParametricDocApprovaleDTO trackingParametricDocApprovaleDTO : listTrackingParametricDocApprovaleDTOs)
			{
				TrackingParamDocApprov trackingParamDocApprov = ParaQueryUtil.getTrackingParamDocApprovById(session, trackingParametricDocApprovaleDTO.getTrackingParamDocApprovId());
				trackingParamDocApprov.setComments(comments);
				if(!approvalStatus.equals("approved"))
				{
					TrackingTaskQaStatus trackingTaskQaStatus = ParaQueryUtil.getTrackingTaskQaStatus(approvalStatus);
					trackingTaskQaStatus.setName(approvalStatus);
					trackingParamDocApprov.setTrackingTaskQaStatus(trackingTaskQaStatus);
					ParaQueryUtil.updateTrackingParamDocApprov(session, trackingParamDocApprov);
				}
			}
		}finally
		{
	...
	private static PartComponent savePart(Supplier supplier, Document document, Pl pl, Session session, String partInfo) throws Throwable
	{
		// check if part is already saved
		// get supplier pl
		SupplierPl supplierPl = ParaQueryUtil.getSupplierPlBySupplierAndPl(pl, supplier, session);
		// part number,family and generic family are separated by comma
		String[] partInfoArr = partInfo.split(",");
		String partNumber = partInfoArr[0];
		String familyStr = partInfoArr[1].equals("null") ? null : partInfoArr[1];
		String genFamilyStr = partInfoArr[2].equals("null") ? null : partInfoArr[2];
		String genericStr = partInfoArr[3].equals("null") ? null : partInfoArr[3];
		String maskStr = partInfoArr[4].equals("null") ? null : partInfoArr[4];
		String convPartStr = partInfoArr[5].equals("null") ? null : partInfoArr[5];

		PartComponent component = ParaQueryUtil.getComponentByPartNumAndSupplierPl(partNumber, supplierPl, session);
		if(component == null)
		{
			component = new PartComponent();
			long id = QueryUtil.getRandomID();
			System.out.println("saving PART with id = " + id + " and  part Number = " + partNumber);
			component.setComId(id);
			component.setStoreDate(new Date());
			component.setPartNumber(partNumber);
		}
		component.setDocument(document);
		component.setSupplierPl(supplierPl);

		component.setUpdateDate(new Date());
//		if(maskStr != null)
//			component.setMask(maskStr);
//		// conversion part is a synonym to alu
//		if(convPartStr != null)
//			component.setAlu(convPartStr);
//		if(genericStr != null)
//			component.setGeneric(genericStr);
		// set family
		if(familyStr != null)
		{
			Family family = ParaQueryUtil.getFamilyByExactName(familyStr, session);
			// if family not found insert new family record
			if(family == null)
			{
				family = DataDevQueryUtil.insertFamily(familyStr, session);
			}
			if(family != null)
		...
		{
			GenericFamily genericFamily = ParaQueryUtil.getGenericFamilyByExactName(genFamilyStr, session);
			if(genericFamily == null)
			{
				genericFamily = DataDevQueryUtil.insertGenericFamily(genFamilyStr, session);
			}
//			if(genericFamily != null)
//				component.setGenericFamily(genericFamily);
	 ...
	 */
	//
	public static List<Document> getParamDevPdfs(long userId, String vendor, String taxonomy, boolean forCS, boolean forFast, boolean forPkg, boolean forTaxonomyTransfer, boolean forDaily, boolean forUpdate) throws Exception
	{
		Session session = null;
		try
		{
			...
			if(forPkg)
				list = ParaQueryUtil.getPkgDevPdfs(userId, session);
			else
				list = ParaQueryUtil.getParamDevPdfs(userId, vendor, taxonomy, forCS, forTaxonomyTransfer, forDaily, forUpdate, forFast, session);
			System.out.println(list.size());
			List<Document> retList = new ArrayList<Document>();
			for(Document documentDB : list)
			{
				Document documentRet = (Document) CloneUtil.cloneObject(documentDB, new ArrayList<String>());
				//
				// Document documentRet = documentDB;

				Set<SupplierPlFamily> supplierPlFamilies = documentDB.getSupplierPlFamilies();
				// documentRet
				// .setSupplierPlFamilies(new HashSet<SupplierPlFamily>(
				// CloneUtil.cloneObjectList(
				// new ArrayList<SupplierPlFamily>(
				// supplierPlFamilies),
				// new ArrayList<String>())));
				documentRet.setSupplierPlFamilies(supplierPlFamilies);
				if(forFast)
				{
					Criteria crit = session.createCriteria(TrackingFast.class);
					crit.add(Restrictions.eq("document", documentDB));
					...
					documentRet.setComponents(componentset);
				}
				else
				{
					Set<PartComponent> components = documentDB.getComponents();

					documentRet.setComponents(components);
				}
				retList.add(documentRet);
			}
			// if(forUpdate && retList.size() > 0)
			// AppContext.noOfUpdateDocs = retList.size();
			return retList;
		}catch(Exception e)
		{
			e.printStackTrace();
	...
	/**
	 * update document with new title then saves fast parts
	 * 
	 * 
	 * @param dataMap
	 * @param supplier
	 ...
	 * @throws Exception
	 */
	public static void saveFastData(Map<Pl, Map<String, Map<String, List<String>>>> plDataMap, Supplier supplier, Document document) throws Exception
	{
		Session session = SessionUtil.getSession();
		QueryUtil.updateDocument(document, session);
		//
		saveFastParts(plDataMap, /* supplier, */document);
	}
	 ...
	 * @throws Exception
	 */
	public static List<TrackingFeedback> getTrackingFeedbackList(long userId, String teamName, int numberOfRows) throws Exception
	{
		Session session = null;
		try
		{
			session = SessionUtil.getSession();
			//
			List<TrackingFeedback> list = ParaQueryUtil.getTrackingFeedbackList(userId, teamName, numberOfRows, session);
			//
			List<TrackingFeedback> retList = new ArrayList<TrackingFeedback>();
			for(TrackingFeedback trackingFeedbackDB : list)
			{
				TrackingFeedback trackingFeedbackRet = (TrackingFeedback) CloneUtil.cloneObject(trackingFeedbackDB, new ArrayList<String>());
				// clone document supplier pl family
				Set<SupplierPlFamily> supplierPlFamilies = trackingFeedbackDB.getDocument().getSupplierPlFamilies();
				trackingFeedbackRet.getDocument().setSupplierPlFamilies(new HashSet<SupplierPlFamily>(CloneUtil.cloneObjectList(new ArrayList<SupplierPlFamily>(supplierPlFamilies), new ArrayList<String>())));
				if(trackingFeedbackRet.getDocument().getPdf().getDocuments() == null)
					trackingFeedbackRet.getDocument().getPdf().setDocuments(new HashSet<Document>());
				trackingFeedbackRet.getDocument().getPdf().getDocuments().add(trackingFeedbackRet.getDocument());
				//
				Set<PartComponent> components = trackingFeedbackDB.getDocument().getComponents();
				if(components != null)
					trackingFeedbackRet.getDocument().setComponents(new HashSet<PartComponent>(CloneUtil.cloneObjectList(new ArrayList<PartComponent>(components), new ArrayList<String>())));
				else
					trackingFeedbackRet.getDocument().setComponents(new HashSet<PartComponent>());

				retList.add(trackingFeedbackRet);
			}
			return retList;
		}catch(Exception e)
		{
	...
	public static void saveFastParts(Map<Pl, Map<String, Map<String, List<String>>>> plDataMap,/*
																								 * Supplier supplier,
																								 */
			Document document) throws Exception
	{
		...
		for(Pl pl : plSet)
		{
			...
			for(String partInfo : keys)
			{
				try
				{
					saveFastPart(document, session, partInfo);
					if(session != null)
		...
		changeParametricTaskStatus(document, "Pending TL Review", null);
	...
	public static boolean movePartsFromSeUrlToAnother(String fromSeUrl, String toSeUrl, long userId) throws Exception
	{
		Session session = SessionUtil.getSession();

		Document fromDocument = ParaQueryUtil.getDocumentBySeUrl(fromSeUrl, session);
		if(fromDocument == null)
			return false;
		Document toDocument = ParaQueryUtil.getDocumentBySeUrl(toSeUrl, session);
		if(toDocument == null)
			return false;
		checkIfTwoDatasheetFromTheSameSupplier(fromDocument, toDocument);

		for(PartComponent component : fromDocument.getComponents())
		{
		...
		validateDocumentsForDsChange(fromDocument, toDocument, userId, session);
		SessionUtil.closeSession(session);
		return true;
	}
	...
	public static boolean movePartToSeUrl(List<PartComponent> components, String seUrl, long loginUser) throws Exception
	{
		Session session = SessionUtil.getSession();
		Document toDocument = ParaQueryUtil.getDocumentBySeUrl(seUrl, session);

		if(toDocument == null)
			return false;

		Document fromDocument = components.get(0).getDocument();

		for(PartComponent component : components)
		{
			// Document fromDocument = component.getDocument();
			checkIfTwoDatasheetFromTheSameSupplier(fromDocument, toDocument);
			System.out.println("toDocument.getId()" + toDocument.getId() + "from document = " + fromDocument.getId());
		...
		validateDocumentsForDsChange(fromDocument, toDocument, loginUser, session);
		SessionUtil.closeSession(session);
		return true;
	}
	...
	public static TrackingParametricDocApprovaleCountDTO getTrackingParametricApprovalValueData(String engName, String task, String approvalStatus, String featureName, String fullValue, String taxonomy, String supplierName, String seUrl,
			Date storeDate, long tlId) throws Exception
	{
		...
		try
		{
			TrackingParametricDocApprovaleCountDTO trackingParametricDocApprovaleCountDTO = new TrackingParametricDocApprovaleCountDTO();
			List<TrackingParametricDocApprovaleDTO> listtraTrackingParametricApprovalValuesDto = new ArrayList<TrackingParametricDocApprovaleDTO>();
			List<TrackingParamDocApprov> listtrTrackingParamDocApprovs = ParaQueryUtil.getTrackingParamDocApprovList(session, engName, task, supplierName, seUrl, approvalStatus, featureName, fullValue, taxonomy, storeDate, tlId);
			String sign = "";
			String value = "";
			String type = "";
			String unit = "";
			...
			for(TrackingParamDocApprov trackingParamDocApprov : listtrTrackingParamDocApprovs)
			{
				...
				approvalValuedto.setTaxonomy(trackingParamDocApprov.getApprovedParametricValue().getPlFeature().getPl().getName());
				approvalValuedto.setFeatureName(trackingParamDocApprov.getApprovedParametricValue().getPlFeature().getFeature().getName());
				List<PartComponent> components = ParametricDevServerUtil.getPartsByApprovedParametricValue(trackingParamDocApprov.getApprovedParametricValue(), trackingParamDocApprov.getApprovedGroup(), 0, 1);
				if(components != null && components.size() > 0)
				{
					approvalValuedto.setPartNumber(components.get(0).getPartNumber());
				}
				else
				{
					approvalValuedto.setPartNumber("");
				}
				...
				String groupFullValue = ParaQueryUtil.getGroupFullValueByGroupId(trackingParamDocApprov.getApprovedGroupId(), session);
				approvalValuedto.setFullValue(groupFullValue);
				approvalValuedto.setComments(trackingParamDocApprov.getComments());
				approvalValuedto.setDDComment(trackingParamDocApprov.getDdComment());
				approvalValuedto.setTrackingParamDocApprovId(trackingParamDocApprov.getId());
				approvalValuedto.setDocument(trackingParamDocApprov.getDocument());
				approvalValuedto.setClientGroupId(trackingParamDocApprov.getApprovedGroupId());
				if(trackingParamDocApprov.getApprovedParametricValue().getPlFeature().getUnit() != null)
					approvalValuedto.setFeatureUnit(trackingParamDocApprov.getApprovedParametricValue().getPlFeature().getUnit().getName());
				...
				List<ApprovedParametricValue> approvedParametricValues = ParaQueryUtil.getApprovedParametricValues(trackingParamDocApprov.getApprovedGroupId(), session);
				if(approvedParametricValues != null && approvedParametricValues.size() > 0)
				{
					for(ApprovedParametricValue approvedParametricValue : approvedParametricValues)
					{
						approvedParametricValue = (ApprovedParametricValue) session.load(ApprovedParametricValue.class, approvedParametricValue.getId());
						if(approvedParametricValue.getFromValue() != null)
						{
							if(!value.trim().isEmpty())
								value = value + "|";
							if(approvedParametricValue.getToValue() != null)
								value = value + approvedParametricValue.getFromValue().getValue() + "|";
							else
								value = value + approvedParametricValue.getFromValue().getValue();
						}
						if(approvedParametricValue.getToValue() != null)
						{
							value = value + approvedParametricValue.getToValue().getValue();
						}

						if(approvedParametricValue.getFromMultiplierUnit() != null && approvedParametricValue.getFromMultiplierUnit().getUnit() != null)
						{
							if(!unit.trim().isEmpty())
								unit = unit + "|";
							if(approvedParametricValue.getToMultiplierUnit() != null && approvedParametricValue.getToMultiplierUnit().getUnit() != null)
								unit = unit + approvedParametricValue.getFromMultiplierUnit().getUnit().getName() + "|";
							else
								unit = unit + approvedParametricValue.getFromMultiplierUnit().getUnit().getName();
						}
						if(approvedParametricValue.getToMultiplierUnit() != null && approvedParametricValue.getToMultiplierUnit().getUnit() != null)
						{
							unit = unit + approvedParametricValue.getToMultiplierUnit().getUnit().getName();
						}
						...
						if(approvedParametricValue.getFromSign() != null)
						{
							...
							if(approvedParametricValue.getToSign() != null)
								sign = sign + approvedParametricValue.getFromSign().getName() + "|";
							else
								sign = sign + approvedParametricValue.getFromSign().getName();
						}
						if(approvedParametricValue.getToSign() != null)
						{
							sign = sign + approvedParametricValue.getToSign().getName();
						}
						...
						if(approvedParametricValue.getFromCondition() != null)
						{
							...
							if(approvedParametricValue.getToCondition() != null)
								condition = condition + approvedParametricValue.getFromCondition().getName() + "|";
							else
								condition = condition + approvedParametricValue.getFromCondition().getName();
						}
						if(approvedParametricValue.getToCondition() != null)
						{
							condition = condition + approvedParametricValue.getToCondition().getName();
						}
						...
						if(approvedParametricValue.getFromValueType() != null)
						{
							...
							if(approvedParametricValue.getToValueType() != null)
								type = type + approvedParametricValue.getFromValueType().getName() + "|";
							else
								type = type + approvedParametricValue.getFromValueType().getName();
						}
						if(approvedParametricValue.getToValueType() != null)
						{
							type = type + approvedParametricValue.getToValueType().getName();
						}

						if(approvedParametricValue.getFromMultiplierUnit() != null && approvedParametricValue.getFromMultiplierUnit().getMultiplier() != null)
						{
							if(!multiplier.trim().isEmpty())
								multiplier = multiplier + "|";
							if(approvedParametricValue.getToMultiplierUnit() != null && approvedParametricValue.getToMultiplierUnit().getMultiplier() != null)
								multiplier = multiplier + approvedParametricValue.getFromMultiplierUnit().getMultiplier().getName() + "|";
							else
								multiplier = multiplier + approvedParametricValue.getFromMultiplierUnit().getMultiplier().getName();
						}
						if(approvedParametricValue.getToMultiplierUnit() != null && approvedParametricValue.getToMultiplierUnit().getMultiplier() != null)
						{
							multiplier = multiplier + approvedParametricValue.getToMultiplierUnit().getMultiplier().getName();
						}
					}
				}
				approvalValuedto.setValue(value);
				approvalValuedto.setSign(sign);
				approvalValuedto.setType(type);
				approvalValuedto.setUnit(unit);
				approvalValuedto.setCondition(condition);
				approvalValuedto.setMultiPlier(multiplier);
				listtraTrackingParametricApprovalValuesDto.add(approvalValuedto);
				value = "";
				sign = "";
				type = "";
			...
			// List<TrackingParametricDocApprovaleDTO> TrackingParametricApprovalValueData =
			// CloneUtil.cloneObjectList(listtraTrackingParametricApprovalValuesDto, new ArrayList<String>());
			// trackingParametricDocApprovaleCountDTO.setListTrackingParametricDocApprovaleDTOs(TrackingParametricApprovalValueData);
			// trackingParametricDocApprovaleCountDTO.setCount(TrackingParametricApprovalValueData.size());
			trackingParametricDocApprovaleCountDTO.setListTrackingParametricDocApprovaleDTOs(listtraTrackingParametricApprovalValuesDto);
			trackingParametricDocApprovaleCountDTO.setCount(listtraTrackingParametricApprovalValuesDto.size());
			return trackingParametricDocApprovaleCountDTO;
		}catch(Exception e)
		{
			e.printStackTrace();
	...
	@SuppressWarnings("unchecked")
	public static List<PartComponent> getPartsByApprovedParametricValue(ApprovedParametricValue val, String fullValue, int from, int max) throws Exception
	{
		final Session session = SessionUtil.getSession();
		List<Long> groupId = new ArrayList<Long>();
		try
		{
			groupId = ParaQueryUtil.getGroupByApprovedParametricValue(val, fullValue, session);
		}catch(Exception e)
		{
			e.printStackTrace();
			return null;
		...
		if(groupId == null || groupId.size() <= 0)
			return null;
		//
		String colName = val.getPlFeature().getColumnName();
		long col = Long.parseLong(colName.substring(colName.lastIndexOf('_') + 1));

		List<PartComponent> partsList = ParaQueryUtil.getComponentList(col, from, max, groupId, session);
		if(partsList == null)
			return null;
		// partsList = CloneUtil.cloneObjectList(partsList,
		// new ArrayList<String>());
		return partsList;
	}
	...
	public static int getPartsCountByApprovedParametricValue(ApprovedParametricValue val) throws Exception
	{
		Session session = null;
		try
		{
			session = SessionUtil.getSession();
			long groupId = ParaQueryUtil.getGroupByApprovedParametricValue(val, session);
			if(groupId <= 0)
				return 0;
			//
			String colName = val.getPlFeature().getColumnName();
			...
			int count = ParaQueryUtil.getPartsCountByPartsParametricColumn(col, groupId, session);
			return count;
		}catch(Exception e)
		{
			getCatchException(e);
...
//				mainDataFeatureValuesDTO.setComponent(component);
	...
	private static List<List<String>> saveUploadExcelSheetDataOfQualityReviewAndExportPackage(List<List<String>> excelDataList) throws Exception
	{
		List<List<String>> unSavedRows = new ArrayList<List<String>>();
		Session session = SessionUtil.getSession();
		List<TrackingFeedback> dtoList = new ArrayList<TrackingFeedback>();
		Session se1 = SessionUtil.getSession();
		for(List<String> row : excelDataList)
		{

			TrackingFeedback trackingFeedback = gettrackingFeedBackBySeUrlAndFeedBackDate(row.get(5), row.get(6), session);

			String comments = row.get(8);
			if(comments == null)
			{
				row.add("Invalid comment");
				unSavedRows.add(row);
			}

			trackingFeedback.setComments(comments);
			String status = row.get(7);

			if(status == null)
			{
				row.add("Invalid status");
				unSavedRows.add(row);
				continue;
			}

			String oldStatus = trackingFeedback.getTrackingTaskStatus().getName();
			System.out.println("comment " + comments + "oldStatus = " + oldStatus);
			if(oldStatus.equals("Pending Assignation"))
			{
				if(status.equals("Assigned") || status.equals("Rejected"))
				{
					trackingFeedback.setTrackingTaskStatus(ParaQueryUtil.getTrackingStatuesByName(status, se1));
					dtoList.add(trackingFeedback);
				}
			}
		}
		...
		try
		{
			saveAlltrackingFeedBack(dtoList, session);
		}catch(ConstraintViolationException e)
		{
			throw new Exception("Duplicated row data");
		}catch(Exception e)
	...
	// * ,
	// */
	// Document document, TrackingFeedback trackingFeedback)
	// throws Exception {
	// boolean isNeedValueApproval = false;
	// String exceptionString = "";
	// Set<Pl> plSet = plDataMap.keySet();
	...
	// // plFeatureName.getFeature().getName());
	// System.out.println("==> Feature = " + pkgFeatureName);
	// //
	// PkgFeature pkgFeature = ParaQueryUtil
	// .getPkgFeatureByExactName(pkgFeatureName, 2,
	// session);
	...
	//
	// if (!isNeedValueApproval)
	// changePkgTaskStatus(document, "Pending TL Review", trackingFeedback);
	...
	public static void insertTrackingParametricObjectByTrackingTaskStautsAndComments(Document document, String Status, String Comments) throws Exception
	{
		Session session = SessionUtil.getSession();
		try
		{
			...
			if(trackingParametric != null)
			{
				...
				TrackingFeedback trackingFeedback = ParaQueryUtil.getTrackingFeedback(trackingParametric.getParametricUserId(), "Parametric", document, session);

				if(trackingFeedback != null)
				{
					if(trackingFeedback.getComments() != null && !trackingFeedback.getComments().trim().equals(""))
						trackingFeedback.setComments(trackingFeedback.getComments() + " | " + Comments);
					else
						trackingFeedback.setComments(Comments);
				...
				else
				{
					trackingFeedback = new TrackingFeedback();
					trackingFeedback.setId(System.nanoTime());
					Date date = trackingParametric.getAssignedDate();
					if(date != null)
					{
					...
					String concatenate = Status + ": " + Comments;
					trackingFeedback.setUserId(trackingParametric.getParametricUserId());
					trackingFeedback.setComments(concatenate);
					trackingFeedback.setDocument(trackingParametric.getDocument());
					trackingFeedback.setFeedbackDate(new Date());
					trackingFeedback.setPl(trackingParametric.getPl());
					trackingFeedback.setPrioriy(trackingParametric.getPrioriy());
					trackingFeedback.setRecieveDate(trackingParametric.getRecieveDate());
					trackingFeedback.setRequestId(trackingParametric.getRequestId());
					trackingFeedback.setTrackingFeedbackType(ParaQueryUtil.getTrackingFeedbackTypeByexactName("PKG", session));
					trackingFeedback.setTrackingTaskStatus(ParaQueryUtil.getTrackingTaskStatus(session, Status));
					trackingFeedback.setTrackingTaskType(trackingParametric.getTrackingTaskType());
					trackingFeedback.setTrackingTeam(ParaQueryUtil.getTrackingTeamByExactName("Parametric", session));
					ParaQueryUtil.insertNewFeedBack(session, trackingFeedback);
				}
			}
		}catch(Exception e)
	...
	public static void validateDocumentsForDsChange(Document formDocument, Document toDocument, long loginUser, Session session) throws Exception
	{
		boolean hasParts = checkIfDocumentHasParts(formDocument, session);
		if(hasParts)
		{
			System.out.println(" this document has parts and i will return with no action     = " + hasParts);
			return;
		}
		TrackingParametric trackingParametric = ParaQueryUtil.getTrackingParametricByDocument(session, formDocument);

		if(trackingParametric != null)
		{
			TrackingTaskStatus trackingTaskStatus = ParaQueryUtil.getTrackingTaskStatusByExactName(session, "Parts Moved");
			trackingParametric.setTrackingTaskStatus(trackingTaskStatus);
			session.saveOrUpdate(trackingParametric);
		}
		// ------------------------------------------------------- now Will work
		// at toDocument ---------
		trackingParametric = ParaQueryUtil.getTrackingParametricByDocument(session, toDocument);

		if(trackingParametric != null)
		{
			if(!trackingParametric.getTrackingTaskStatus().getName().equals("Pending TL Review"))
			{
				TrackingTaskStatus trackingTaskStatus = ParaQueryUtil.getTrackingTaskStatusByExactName(session, "Pending TL Review");
				trackingParametric.setTrackingTaskStatus(trackingTaskStatus);
				session.saveOrUpdate(trackingParametric);
			}
		}
		else
		{
			trackingParametric = new TrackingParametric();
			trackingParametric.setId(System.nanoTime());
			trackingParametric.setDocument(toDocument);
			TrackingTaskStatus trackingTaskStatus = ParaQueryUtil.getTrackingTaskStatusByExactName(session, "Pending TL Review");
			trackingParametric.setTrackingTaskStatus(trackingTaskStatus);
			TrackingTaskType taskType = ParaQueryUtil.getTrackingTaskTypeByName("New", session);
			trackingParametric.setTrackingTaskType(taskType);

			trackingParametric.setParametricUserId(loginUser);

			TrackingTransferStatus trackingTransferStatus = ParaQueryUtil.getTrackingTransferStatus(session, "Pending Parametric");

			trackingParametric.setTrackingTransferStatus(trackingTransferStatus);
			Pl pl = null;
			for(SupplierPlFamily supplierPlFamily : toDocument.getSupplierPlFamilies())
			{
				pl = supplierPlFamily.getPl();
				break;
			}
			trackingParametric.setPl(pl);
