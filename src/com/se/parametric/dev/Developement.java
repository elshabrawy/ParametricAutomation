...
public class Developement extends JPanel implements ActionListener
{
	...
	JButton save, validate, separation, separationSave, srcFeedbackBtn, AutoFill;
	...
	public void actionPerformed(ActionEvent event)
	{
		...
		if(event.getActionCommand().equals("LoadSheet"))
		{
			...
			else
			{
				...
				// autoFillProcess.setNormalFlagData();
				// autoFillProcess.setFormulationFlagData();
				// autoFillProcess.setRangeFlagData();
				// autoFillProcess.setConverterFlagData();
			}
		...
		else if(event.getSource() == filterPanel.filterButton)
		{
			...
			String extracted = filterPanel.comboBoxItems[3].getSelectedItem().toString();
			String priority = filterPanel.comboBoxItems[4].getSelectedItem().toString();
			tablePanel.selectedData = DataDevQueryUtil.getReviewPDF(new Long[] { userId }, plName, supplierName, taskType, extracted, startDate, endDate, null, "assigned", priority, StatusName.assigned, null);
