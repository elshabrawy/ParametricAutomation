...
public class ConditionValidator extends ValidationsCommentsStatus implements
		Validator {
//	MacroRunner macro = new MacroRunner();
	...
	public boolean Validat(RelatedFeaturesDTO datarow) {
		...
		if (Validate(datarow,datarow.getIndependent(), datarow.getQachecksrelatedfet())) {
			if (datarow.getDependentFeatures().size() > 0) {
				validate = validateSempleDependentFeatures(datarow,datarow
						.getSheetrow().getFeatures(),
						datarow.getDependentFeatures());
	...
	private boolean Validate(RelatedFeaturesDTO datarow,FeatureDTO fet,
			QaChecksInDependentFeature qachecksindependent) {
		boolean validate = true;
		int operationtype = qachecksindependent.getQaChecksValidatetype()
				.getId().intValue();
...
//        valid = macro.formulaExcution(formula);
	...
	private boolean validateContans(FeatureDTO fet,
			QaChecksInDependentFeature qachecksindependent){
		boolean valid = true;
	...
	private boolean ValidateCondition(FeatureDTO fet,
			QaChecksInDependentFeature qachecksindependent) {
		boolean valid = true;
		String fetnm = fet.getFeatureName();
...
//			}
	...
	private boolean validateSempleDependentFeatures(RelatedFeaturesDTO datarow ,Map sheetFeatures,
			Map dependentFeaturesfromdb) {
		Iterator sheetfeaturesiter = dependentFeaturesfromdb.keySet()
				.iterator();
		boolean valid = true;
		String fetnm;
		String operandone;
		...
		while (sheetfeaturesiter.hasNext()) {
			...
			if (!Validate(datarow,(FeatureDTO) sheetFeatures.get(fetnm),
					(QaChecksInDependentFeature) dependentFeaturesfromdb
							.get(fetnm))) {
